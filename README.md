...
Класс будет использован, как контейнер карточек товара, в котором они будут отображаться, на вход принимает HTML-элемент, в котором будут отображаться карточки товаров (в контексте проектной это может быть либо тег `<main class="gallery">`, либо элемент списка из корзины `<ul class="basket__list">`). Из методов использует сеттер для поля `cardsCatalog: HTMLElement[]`, который будет добавлять элементы карточек в контейнер, метод `render` наследует, но не модифицирует.
